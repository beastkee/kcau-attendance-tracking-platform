...
/**
 * Get intervention escalation rules
 */
export function shouldEscalateIntervention(intervention: Intervention): boolean {
  // Escalate if:
  // 1. High risk and in-progress for more than 2 weeks
  // 2. Multiple interventions triggered
  // 3. Risk score increased after intervention

  const triggeredTime = new Date(intervention.triggeredAt).getTime();
  const now = new Date().getTime();
  const twoWeeksMs = 14 * 24 * 60 * 60 * 1000;

  if (intervention.status === 'in-progress' && now - triggeredTime > twoWeeksMs) {
...
/**
 * Generate email alert content for teachers
 */
export function generateTeacherAlertEmail(
  studentName: string,
  ...
  return {
    subject: `⚠️ High-Risk Student Alert: ${studentName}`,
    body: `
Dear Teacher,
...
**Recommended Actions:**
1. Schedule a meeting with the student to discuss attendance
2. Explore any underlying issues (personal, health, academic)
3. Establish an attendance improvement plan with clear goals
4. Follow up with the student weekly

Please log your intervention notes in the system so we can track progress.

Best regards,
KCAU Attendance Tracking System
    `.trim(),
  };
}

/**
 * Generate email alert content for counselors
 */
export function generateCounselorAlertEmail(
  studentName: string,
  interventionReason: string,
  riskScore: number
): {
  ...
  return {
    subject: `🚨 Urgent: Counselor Referral Required - ${studentName}`,
    body: `
Dear Academic Counselor,

A student has been escalated to your attention due to high-risk attendance patterns.

**Student:** ${studentName}
**Risk Score:** ${riskScore}/100
**Critical Concern:** ${interventionReason}

**Recommended Actions:**
1. Schedule an urgent meeting with the student
2. Assess for underlying issues (personal, health, family, financial)
3. Develop a comprehensive academic intervention plan
4. Coordinate with parents/guardians if necessary
5. Consider academic accommodations or support services

This referral indicates the student may be at risk of dropping out.
Your immediate attention is critical.

Please update the intervention status in the system after meeting with the student.
...
/**
 * Calculate intervention effectiveness (improvement after intervention)
 */
export function calculateInterventionEffectiveness(
  riskBeforeIntervention: number,
  riskAfterIntervention: number
): {
  improved: boolean;
  improvementPercentage: number;
  status: 'success' | 'partial' | 'ineffective';
} {
  const improvementPercentage = ((riskBeforeIntervention - riskAfterIntervention) / riskBeforeIntervention) * 100;

  let status: 'success' | 'partial' | 'ineffective' = 'ineffective';
  ...
  return {
    improved: improvementPercentage > 0,
